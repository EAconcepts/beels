...
const MobileSideBar = () => {
  ...
  useEffect(() => {
    ...
    const secondPaths = ["/"];
    const thirdPaths = ["/dashboard/add-ambassador"];
    const fourthPaths = ["/tasks/all", "/tasks/personal/all"];

    const fifthPaths = ["/task/create"];
  ...
  return (
    <div className="bg-[#001A04] w-full min-h-screen pt-10 lg:hidden">
      ...
      <div className="flex justify-start gap-4 items-center mt-5 px-2 ">
        <p className="text-[#FFFFFF] text-[16px] max-lg:text-sm max-sm:text-xs font-[700] font-[Inter] ">
          {" "}
          {user?.first_name + "" + user?.last_name}{" "}
        </p>
        <RiArrowDropDownLine className="text-white" />
      </div>
      <p className="text-[#FFFFFF] text-[12px]  max-md:text-[10px] font-[400] font-[Inter] mt-5 px-2 ">
        {" "}
        {user?.type}{" "}
      </p>
      <div
        className={`flex justify-start hover:cursor-pointer gap-4 items-center px-2 my-4 py-2 ${
          activeLink === "dashboard" ? "bg-[#3AB54A]" : ""
        }`}
        onClick={handleDashboardClick}
      >
        <FaHouseDamage className="text-white" />
        <p className="text-[#FFFFFF] text-[16px] max-lg:text-sm max-sm:text-xs font-[600] font-[Inter] ">
          {" "}
          Dashboard{" "}
        </p>
      </div>
      <div className="flex justify-start hover:cursor-pointer gap-4 items-center px-2 my-4 py-2">
        <div className="flex justify-start items-center gap-4">
          <img src={TaskIcon} />
          <p className="text-sm font-[700] text-[#FFFFFF] font-[Rockwell]">
            {" "}
            Ambassadors
          </p>
        </div>
      ...
      {showAmbassadors && (
        <div>
          <div
            className={`flex hover:cursor-pointer justify-start gap-4 items-center px-3 my-3 py-2 ${
              secondActiveLink === "ViewAmbassadors" ? "bg-[#3AB54A]" : ""
            }`}
            onClick={handleViewAmbassadorsClick}
          >
          ...
          <div
            className={`flex hover:cursor-pointer justify-start gap-4 items-center px-3 py-2 my-3 ${
              thirdActiveLink === "AddAmbassadors" ? "bg-[#3AB54A]" : ""
            } `}
            onClick={() => {
              navigate("/dashboard/add-ambassador");
              setThirdActiveLink("AddAmbassadors");
            }}
          >
            <div className="w-4 h-4 rounded-full border border-[#FFFFFF]"></div>
      ...
      {tasks && (
        <div>
          <div
            className={`flex hover:cursor-pointer justify-start gap-4 items-center px-3 my-3 py-2 ${
              fourthActiveLink === "viewTask" ? "bg-[#3AB54A]" : ""
            }`}
          ...
          <div
            className={`flex hover:cursor-pointer justify-start gap-4 items-center px-3 py-2 my-3 ${
              fifthActiveLink === "createTask" ? "bg-[#3AB54A]" : ""
            }`}
      ...
      <div
        className={`flex hover:cursor-pointer justify-start gap-4 hover:bg-[#3AB54A] items-center px-2 mt-10  py-2 `}
