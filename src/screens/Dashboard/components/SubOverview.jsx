...
const SubOverview = () => {
  ...
  // const [recipient, setEmails] = useState("")
  const handleChange = (e) => {
    const { name, value } = e.target;
    setEmailInfo((prevVals) => ({ ...prevVals, [name]: value }));
  };
  ...
  return (
    <div className="w-full flex flex-col lg:bg-[#FAFAFA] bg-[#F1F1F1] pt-[37px]">
      ...
      <div className="w-full bg-[#E9E9E9] mt-[11px] flex ustify-between px-[9px] gap-x-[32px] max-xsm:gap-x-[16px] justify-between items-center ">
        ...
        <button
          onClick={() =>
            handleCopy(ambassadorQuery.data?.data?.data?.user?.ref_code)
          }
          className="bg-[#082C25] w-fu max-xsm:px-[14px] max-xsm:text-[10px] shrink- py-[12px] px-[20px] rounded-[8px] xsm:text-[12px] font-[400] text-white leading-[20.3px] text-[14px]"
        >
          Copy Link
        </button>
      </div>
      ...
      <form onSubmit={handleSendEmail} className="mt-[33px] flex flex-col">
...
"
        ...
        <div className="px-[46px] mt-[22px] pb-[32px]">
          <button
            ...
            className="w-full bg-[#082C25] disabled:bg-opacity-45 p-[10px] rounded-[8px] leading-[29px] text-[20px] font-[400] font-inter text-white text-center"
          >
            {" "}
            Send Email
          </button>
